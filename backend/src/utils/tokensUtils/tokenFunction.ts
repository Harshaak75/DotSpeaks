...
export async function SaveClientRefreshToken(
  userId: string,
  refreshToken: string
) {
  try {
    // const hashedRefreshToken = jwt.sign({ token: refreshToken }, process.env.REFRESH_SECRET as string);
    const expires = new Date(Date.now() + 1000 * 60 * 60 * 24 * 20); // 20 days
    console.log("resfresh client side: ", refreshToken);
    await prisma.clientRefreshToken.upsert({
...
export const VerifyRefreshToken = async (refreshtoken: string) => {
  try {
    ...
    if (role == "CLIENT") {
      tokenRecord = await prisma.clientRefreshToken.findUnique({
        where: { clientId: userId },
      });
    } else {
