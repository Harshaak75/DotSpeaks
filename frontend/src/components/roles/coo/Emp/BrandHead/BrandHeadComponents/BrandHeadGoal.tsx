...
const BrandHeadGoals = () => {
    ...
    return (
        <div className="p-8 bg-gray-50 min-h-screen">
            <h1 className="text-3xl font-bold text-gray-900 mb-6">Brand Head: Goal Delegation</h1>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-8">
                <div className="flex items-start">
                    ...
                    <div>
                        <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Your Weekly Goal (from Project Manager)</h2>
                        <p className="text-xl font-bold text-gray-800 mt-1">{projectManagerWeeklyGoal.title}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                <div className="flex justify-between items-center mb-4">
                    <div>
                        ...
                        <p className="text-sm text-gray-500">Break down the weekly goal into daily tasks and assign them to your team members.</p>
                    </div>
                ...
                <div className="space-y-4">
                    {dailyTasks.map(task => {
                        ...
                        return (
                            <div key={task.id} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-start p-4 bg-gray-50 rounded-lg border">
                                <div className="md:col-span-2">
                                    <label className="block text-xs font-medium text-gray-500">Task Description</label>
                                    <textarea 
                                ...
                                <div>
                                    <label className="block text-xs font-medium text-gray-500">Assign Team</label>
                                    <select 
                                ...
                                <div>
                                    ...
                                    <select 
                                        ...
                                        className="mt-1 w-full p-2 border-gray-300 rounded-md text-sm disabled:bg-gray-200"
                                    >
                                        <option value="">Select Member...</option>
                                        {selectedTeam?.members.map(member => <option key={member.id} value={member.id}>{member.name} ({member.role})</option>)}
                                    </select>
                                </div>
                            </div>
                        );
